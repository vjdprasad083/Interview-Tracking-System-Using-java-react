...
public class TechPanelServiceImpl implements TechPanelService {
	...
	@Override
	public InterviewSchedule giveTechRating(Integer interviewScheduleId,Integer rating) throws InterviewNotFoundException {
		...
		if(interview.isPresent()) {
			...
			interview.get().setTechRating(rating);
	...
	@Override
	public List<InterviewSchedule> getTechInterviews(Integer panelMemberId)throws DetailsNotFoundException {
		...
		if(p.isPresent()) {
			...
			interviewScheduleRepo.findAll().stream().filter(list-> list.getPanelMember().equals(p.get())).forEach(s -> interviews.add(s));		if(interviews.isEmpty()) {
